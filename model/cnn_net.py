...
from keras.layers import Input, Activation, ZeroPadding2D, BatchNormalization, Conv2D, Deconv2D, MaxPooling2D, \
    Concatenate, Add, UpSampling2D, LeakyReLU, GlobalAveragePooling2D, Dense, Lambda
...
def discriminator(patch_height=256, patch_width=256, channels=313):
    """
    ...
    inputs = Input((patch_height, patch_width, channels+1))
    ...
    d.compile(optimizer=Adam(lr=1e-4), loss=d_loss, metrics=['accuracy'])

    return d


def get_gan(g, d, prior_probs):
    """
    GAN (that binds generator and discriminator)
    It gets the combined and compiles model (graph + discriminator)
    :param g: colorization model
    :param d: discriminative model
    :return: the combined model (graph + discriminator)
    """
    ...
    def gan_loss(y_true, y_pred):
        ...
        L_col = categorical_crossentropy_weighted(labels, fake_labels)

        return alpha_recip * L_adv + L_col

    gan.compile(optimizer=Adam(lr=1e-4), loss=gan_loss, metrics=['accuracy'])
